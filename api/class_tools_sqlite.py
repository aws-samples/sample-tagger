        ...
        try:
            with self._get_connection() as conn:
                ...
                for i in range(0, len(tags), batch_size):
                    ...
                    batch_data = []
                    for tag in batch:
                        # Serialize tags and metadata to JSON strings
                        tags_json = json.dumps(tag['tags']) if tag['tags'] else '{}'
                        metadata_json = json.dumps(tag['metadata'], default=str) if tag['metadata'] else '{}'
                        ...
                        try:
                            ...
                            self.logger.error(f"Invalid JSON for resource {tag['resource_id']}: {e}")
                            # Use empty JSON objects as fallback
                            tags_json = '{}'
                            metadata_json = '{}'
                        ...
                        batch_data.append((
                            ...
                            tags_json,  # Will be converted to JSON by json() function
                            tag['tags_number'],
                            metadata_json,  # Will be converted to JSON by json() function
                            tag['arn'],
                        ))
